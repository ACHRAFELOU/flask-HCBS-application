...
# CENTRES BIEN POSITIONNÉS POUR GARDER LA FORME CIRCULAIRE
# On décale vers la droite mais en gardant assez de marge
margin_right = 100  # Marge à droite pour éviter que le cercle soit coupé
margin_left = 450   # Espace pour le panneau à gauche

centre_gauche = (margin_left + 350, screen_height // 2 + 20)
centre_droit = (margin_left + 350 + 500, screen_height // 2 + 20)
...
def on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        # Dans la fonction on_mouse, modifier la partie d'affichage du tooltip :
        if display_info:
            ...
            (tw, th), _ = cv2.getTextSize(txt, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)  # Taille augmentée
...
def draw_info_panel(img, x, y, width, height,
                    ...
                    alert=False):
    # Créer un sous-image pour le panneau
    ...
    # Configuration des colonnes - ESPACEMENT AUGMENTÉ
    col_w = (width - 60) // 2
    col_x = [30, 30 + col_w + 20]  # Plus d'espace entre colonnes
    start_y = title_y + 45  # Plus d'espace après le titre
    line_height = 30  # Hauteur de ligne augmentée

    # --- Colonne 1: Sein Gauche ---
    current_y = start_y
    cv2.putText(panel, "SEIN GAUCHE", (col_x[0], current_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 200), 2)  # Taille augmentée
    current_y += line_height

    cv2.putText(panel, f"Min: {min_g:.1f}C", (col_x[0], current_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)  # Épaisseur augmentée
    current_y += line_height
    ...
    # Barre de progression pour sein gauche
    bar_y = current_y + 12
    bar_width = col_w - 20
    ...
    # Barre de progression pour sein droit
    bar_y = current_y + 12
    ...
    # Copier le panneau sur l'image principale
...
while True:
    ...
    # Position de la colorbar entre les deux seins
    ...
    # ---------------- Colorbar centrale - TEXTE AGRANDI ----------------
    ...
    # ---------------- Panneau info À GAUCHE - TAILLE AUGMENTÉE ----------------
    panel_width = 500  # Largeur augmentée
    panel_height = 450  # Hauteur augmentée
    panel_x = 20  # Marge réduite
    ...
    # Indiquer dernière sauvegarde - TEXTE AGRANDI

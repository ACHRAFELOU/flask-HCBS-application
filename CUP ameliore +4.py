...
lissage_sigma = 1.2
zoom_state = 0  # 0: aucun zoom, 1: premier zoom affiché, 2: deux zooms affichés
...
def detecter_zones_asymetrie(grid_g, grid_d, seuil_asym=0.3):
...
def draw_info_panel(img, x, y, width, height,
    ...
    if nb_zones_asym_g > 0 or nb_zones_asym_d > 0:
        cv2.putText(panel, f"Zones asymetriques: {nb_zones_asym_g + nb_zones_asym_d}",
...
while True:
    ...
    seuil_asymetrie = 0.3  # Seuil d'asymétrie en °C
    ...
    gz_smooth_g_clean, mask_roi_g_clean, offset_g_clean = render_heatmap_on_image(
        img_clean, centre_gauche, gx_g, gy_g, gz_g, mask_g, tmin, tmax, gamma=gamma_track)
    gz_smooth_d_clean, mask_roi_d_clean, offset_d_clean = render_heatmap_on_image(
        img_clean, centre_droit, gx_d, gy_d, gz_d, mask_d, tmin, tmax, gamma=gamma_track)
